...
Tile::Tile(QWidget *parent) : QWidget(parent) {
  ...
  connect(this, SIGNAL(tileStartDrag(int, int)), parent,
          SLOT(onTileGrabbed(int, int)));
  connect(this, SIGNAL(tileEndDrag(int, int, int, int)), parent,
          SLOT(onTileReleased(int, int, int, int)));
  connect(this, SIGNAL(tileMoveDrag(const QPoint &)), parent,
          SLOT(updateCursor(const QPoint &)));
}
Tile::~Tile() {}
...
void Tile::mouseMoveEvent(QMouseEvent *event) {
  if (event->buttons() & Qt::LeftButton
      && (dragStartPos - event->pos()).manhattanLength() > 10) {
    QDrag *drag = new QDrag(this);
    // using Mime Data to transfer a text of two digits row and column without
    // any spaces
    QMimeData *data = new QMimeData;
    data->setText(
        QString::fromStdString(std::to_string(row) + std::to_string(col)));
