...
private:
  ...
  std::pair<int, int> findKing(bool colour) const;
  Tile safeGet(int row, int col);
  bool piecePresence(int row, int col, Piece p, bool colour);
  TileSet possiblePieces(Tile *t) const;
  TileSet possiblePieces(Tile *t, Piece p) const;
...
public:
  ...
  Tile const *get_c(int row, int col) const;
  Tile const *get_c(std::string str) const;
  Tile const *get_c(std::pair<int, int> coords) const;
  bool getTurn();
