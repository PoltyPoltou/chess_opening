...
static const Tile STD_BOARD[8][8] = {
    {Tile(ROOK, true), Tile(KNIGHT, true), Tile(BISHOP, true),
     Tile(QUEEN, true), Tile(KING, true), Tile(BISHOP, true),
     Tile(KNIGHT, true), Tile(ROOK, true)},
    {Tile(PAWN, true), Tile(PAWN, true), Tile(PAWN, true), Tile(PAWN, true),
     Tile(PAWN, true), Tile(PAWN, true), Tile(PAWN, true), Tile(PAWN, true)},
    {Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile()},
    {Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile()},
    {Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile()},
    {Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile(), Tile()},
    {Tile(PAWN, false), Tile(PAWN, false), Tile(PAWN, false), Tile(PAWN, false),
     Tile(PAWN, false), Tile(PAWN, false), Tile(PAWN, false),
     Tile(PAWN, false)},
    {Tile(ROOK, false), Tile(KNIGHT, false), Tile(BISHOP, false),
     Tile(QUEEN, false), Tile(KING, false), Tile(BISHOP, false),
     Tile(KNIGHT, false), Tile(ROOK, false)}};

BoardState::BoardState() :
    castlingRight_long {true, true},
...
Tile const *BoardState::get_c(int row, int col) const {
  return &board[row][col];
}

Tile const *BoardState::get_c(std::string str) const {
  return get_c(str[1] - '1', str[0] - 'a');
}
Tile const *BoardState::get_c(std::pair<int, int> coords) const {
  return get_c(coords.first, coords.second);
}
...
std::string BoardState::playMove(Move const &mv) {
  ...
  if (played->piece() == Piece::KING
      && (uci == "e1c1" || uci == "e1g1" || uci == "e8c8" || uci == "e8g8")) {
...
/**
    \brief Check if the piece p of colour clr at coords t1 can go to coords t2
   with a legal movement Does not handle castling \param attacking checks if the
   tile is attacked (ie it will not check anything about t2)
*/
bool BoardState::legalPath(Piece p,
                           bool clr,
  ...
  if (!get_c(t2)->is_empty() && get_c(t2)->color() == clr && !attacking) {
    return false;
  }
  switch (p) {
    case ROOK: {
    ...
    case BISHOP: {
      if (r1 + c1 == r2 + c2 || r1 - c1 == r2 - c2) {
        ...
        while (r != r2
               && c != c2) // both condition should go off at the same time
        {
          if (!get_c(r, c)->is_empty()) {
            return false;
          }
          r += rDir;
          c += cDir;
    ...
    case KNIGHT:
      return abs(r1 - r2) + abs(c1 - c2) == 3 && r1 != r2
             && c1 != c2; // circle of radius 3 but can't be on a liine
      break;
    ...
    case PAWN: {
      ...
      bool firstPush =
          c1 == c2
          && ((clr == WHITE && r1 == 1 && r2 == 3 && get_c(2, c1)->is_empty())
              || (clr == BLACK && r1 == 6 && r2 == 4
                  && get_c(5, c1)->is_empty()));
      if (r2 - r1 == (clr == WHITE) - (clr == BLACK)) {
        ...
        takingMove =
            !stdMove
            && abs(c1 - c2)
                   == (!get_c(t2)->is_empty() && get_c(t2)->color() != clr);
      }
      if (!attacking) {
        return takingMove || (get_c(t2)->is_empty() && (stdMove || firstPush));
      } else {
        return (r2 - r1 == (clr == WHITE) - (clr == BLACK))
               && abs(c1 - c2) == 1;
      }
    } break;
...
*/
std::pair<int, int> BoardState::findKing(bool colour) const {
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (get_c(i, j)->piece() == KING && get_c(i, j)->color() == colour) {
        return std::make_pair(i, j);
      }
    }
  }
  std::cerr << "findKing was used on a BoardState that has no king"
            << std::endl;
...
bool BoardState::piecePresence(int row, int col, Piece p, bool colour) {
  ...
  return !pieceTile.is_empty() && pieceTile.piece() == p
         && pieceTile.color() == colour;
}
...
/**
 * \brief checks if the tile at coords is attacked by a piece of the given
 * colour
 */
bool BoardState::isAttacked(std::pair<int, int> coords, bool colour) {
  for (int r = 0; r < 8; r++) {
    for (int c = 0; c < 8; c++) {
      Tile *t = get(r, c);
      if (!t->is_empty() && t->color() == colour
          && legalPath(t->piece(), t->color(), std::make_pair(r, c), coords,
                       true)) {
        return true;
      }
    }
...
bool BoardState::isLegal(Move const &mv) {
  ...
  if (!t1->is_empty() && t1->color() == turn && mv.getStart() != mv.getEnd()) {
    // We check that the piece can move from t1 to t2 without looking at checks,
    // pin etc...
    ...
    switch (t1->piece()) {
      case PAWN: {
        ...
        // promotion check
        bool promotion =
            (!mv.is_promotion() && (r2 != 7 && r2 != 0))
            || // last row ? (no colour check as the rules won't allow the other
               // color)
            ((mv.getPromotion() == KNIGHT)
             || // checks that the promotion is valid
             (mv.getPromotion() == BISHOP) || (mv.getPromotion() == ROOK)
             || (mv.getPromotion() == QUEEN) && mv.is_promotion());
      ...
      case KING: {
        ...
        if (!standardMove) {
          if (mv.getUci() == "e1c1") {
            castleMove = !isCheck() && get("b1")->is_empty()
                         && get("c1")->is_empty() && get("d1")->is_empty()
                         && !isAttacked(std::make_pair(0, 3), !t1->color())
                         && castlingRight_long[turn];
          }
          if (mv.getUci() == "e8c8") {
            castleMove = !isCheck() && get("b8")->is_empty()
                         && get("c8")->is_empty() && get("d8")->is_empty()
                         && !isAttacked(std::make_pair(7, 3), !t1->color())
                         && castlingRight_long[turn];
          }
          if (mv.getUci() == "e1g1") {
            castleMove = !isCheck() && get("f1")->is_empty()
                         && get("g1")->is_empty()
                         && !isAttacked(std::make_pair(0, 5), !t1->color())
                         && castlingRight_short[turn];
          }
          if (mv.getUci() == "e8g8") {
            castleMove = !isCheck() && get("f8")->is_empty()
                         && get("g8")->is_empty()
      ...
      default:
        movePossible =
            legalPath(t1->piece(), t1->color(), mv.getStart(), mv.getEnd());
        break;
    }
